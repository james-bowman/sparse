...
// Usmm (Sparse matrix multiply (C <- alpha * A * B + C Or C <- alpha * A^T * B + C))
...
func Usmm(transA bool, k int, alpha float64, a *SparseMatrix, b []float64, ldb int, c []float64, ldc int) {
	// A is m x n, B is n x k, C is m x k
	// Perform k matvecs: i-th column of C gets A*(i-th column of B)
	for i := 0; i < k; i++ {
		Usmv(transA, alpha, a, b[i:], ldb, c[i:], ldc)
	}
}
